...
# %%
dats = [dat_down, dat_down50, dat_up, dat]
...
fig, ax = plt.subplots(1, 1)
ax.contourf(y, Z[70:], T[70:, :], np.arange(0.6, 1.15, 0.05), extend='both', cmap='inferno')
ax.fill_between(y, ax.get_ylim()[0], -grid.Depth[iy0:iy1, 0], color='grey')
ax.plot(y, z, lw=3, zorder=2)
ax.plot(y, zs, lw=3, zorder=1)
...
iy1 = 2000
it0 = -100
Tcs = np.arange(0.7, 0.91, 0.01)
models = [dat_down, dat_down50, dat, dat_up]
...
# %%
mnames = ['down 100 m', 'down 50 m', 'no change', 'up 100 m']
...
axs[0].plot(A[:, :-1], zcs)
axs[1].plot(Vi[:, :-1], zcs, label='Vi')
axs[1].plot(Vo[:, :-1], zcs, label='Vo')
axs[2].plot(w[:, :-1], zcs)
axs[3].plot(Tr[:, :-1], zcs)
